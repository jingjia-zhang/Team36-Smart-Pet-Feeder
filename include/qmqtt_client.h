...
#include <iostream>  // Include the iostream library for input and output stream operations (e.g., cout, cin)
...
private:
    ...
    const std::string hostUrl = "47.122.67.6";  // Declare a constant string to store the URL of the MQTT broker
    const int hostPort = 1883;  // Declare a constant integer to store the default port number (1883) for the MQTT broker connection
    const std::string userName = "test";  // Declare a constant string to store the default username ("test") for MQTT broker authentication
    const std::string passWorld = "test1234";  // Declare a constant string to store the default password ("test1234") for MQTT broker authentication
