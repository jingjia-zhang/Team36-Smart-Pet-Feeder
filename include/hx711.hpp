#include <wiringPi.h>  // Include the controlling GPIO pins on Raspberry Pi.
#include <iostream>  // Include the For input/output stream operations.
#include <fstream>  // Include the file handling (reading/writing calibration data).
#include <chrono>  // Include the time-related functions
#include <thread>  // Include the thread management (used with std::this_thread::sleep_for)
#include <mutex>  // Include the thread synchronization to ensure safe access to shared resources
#include <queue>  // Include the using priority queues to filter sensor data
#include <cmath>  // Include the mathematical operations
class HX711
{
public:
    ...
    {
        std::cout << "Start calibration, please remove all weights..." << std::endl;  // Display a message prompting the user to remove all weight before calibration
        ...
        std::cout << "Please place known weight:" << known_weight << "g" << std::endl;  // Prompt the user to place a known weight on the scale for calibration
        ...
        std::cout << "Calibration complete! Scale factor:" << scale << std::endl;  // Display a message indicating that calibration is complete and show the calculated scale factor
    }
...
private:
    ...
    // Save the calibration data (offset and scale) to a file for future use
    ...
    // Load the calibration data (offset and scale) from a file to apply previously
...
//         std::cout << "Current weight:" << weight << " g" << std::endl;
